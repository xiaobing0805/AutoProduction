...
var CodeMirror = (function() {
	...
	function CodeMirror(place, givenOptions) {
		...
		wrapper.innerHTML = '<div style="overflow: hidden; position: relative; width: 3px; height: 0px;">'
				...
				'<div style="position: relative; z-index: -1"></div><div></div>'
		...
		function onDragStart(e) {
			...
			if (gecko || chrome) {
				...
				img.scr = 'data:image/gif;base64,R0lGODdhAgACAIAAAAAAAP///ywAAAAAAgACAAACAoRRADs='; // 1x1
																									// image
		...
		function handleKeyBinding(e) {
			...
			if (next && !isModifierKey(e))
				maybeTransition = setTimeout(function() {
					if (getKeyMap(options.keyMap) == startMap) {
						options.keyMap = (next.call ? next.call(null, instance)
								: next);
					}
				}, 50);
			...
			if (e_prop(e, "shiftKey")) {
				handled = lookupKey("Shift-" + name, options.extraKeys,
						options.keyMap, function(b) {
							return doHandleBinding(b, true);
						})
						|| lookupKey(name, options.extraKeys, options.keyMap,
								function(b) {
									if (typeof b == "string"
											&& /^go[A-Z]/.test(b))
				...
				handled = lookupKey(name, options.extraKeys, options.keyMap,
						doHandleBinding);
			}
			if (handled) {
				e_preventDefault(e);
		...
		function onKeyDown(e) {
			...
			if (window.opera) {
				...
				// Opera has no cut event... we try to at least catch the key
				// combo
				if (!handled && code == 88
		...
		function onKeyPress(e) {
			...
			if (((window.opera && !e.which) || khtml) && handleKeyBinding(e))
				return;
			var ch = String.fromCharCode(charCode == null ? keyCode : charCode);
			if (options.electricChars && mode.electricChars
					&& options.smartIndent && !options.readOnly) {
		...
		function scrollEditorIntoView() {
			...
			// IE returns bogus coordinates when the instance sits inside of an
			// iframe and the cursor is hidden
		...
		function updateDisplay(changes, suppressCallback) {
			...
			function checkHeights() {
				...
				if (heightChanged)
					code.style.height = (doc.height * th + 2 * paddingTop())
							+ "px";
			...
			if (different || gutterDirty) {
				// If the gutter grew in size, re-check heights. If those
				// changed, re-draw gutter.
				updateGutter() && options.lineWrapping && checkHeights()
						&& updateGutter();
			}
			updateSelection();
								...
								if (!nextIntact || nextIntact.from > j) {
									if (line.hidden)
										...
										var html = '<pre'
												...
												+ '</pre>';
										// Kludge to make sure the styled
										// element lies behind the selection (by
										// z-index)
										if (line.bgClassName)
											html = '<div style="position: relative"><pre class="'
													+ line.bgClassName
													+ '" style="position: absolute; left: 0; right: 0; top: 0; bottom: 0; z-index: -2">&#160;</pre>'
													+ html + "</div>";
									}
									scratch.innerHTML = html;
									lineDiv.insertBefore(scratch.firstChild,
											curNode);
								} else {
									curNode = curNode.nextSibling;
								}
		...
		function updateGutter() {
			...
			if (normalNode != null) {
				...
				while (val.length + pad.length < minwidth)
					pad += "\u00a0";
				if (pad)
					node.insertBefore(document.createTextNode(pad),
							node.firstChild);
			}
			gutter.style.display = "";
			var resized = Math.abs((parseInt(lineSpace.style.marginLeft) || 0)
					- gutter.offsetWidth) > 2;
		...
		function updateSelection() {
			...
			inputDiv.style.top = Math.max(0, Math.min(scroller.offsetHeight,
					headPos.y + lineOff.top - wrapOff.top))
					+ "px";
			inputDiv.style.left = Math.max(0, Math.min(scroller.offsetWidth,
					headPos.x + lineOff.left - wrapOff.left))
					+ "px";
			if (collapsed) {
				cursor.style.top = headPos.y + "px";
				cursor.style.left = (options.lineWrapping ? Math.min(headPos.x,
						lineSpace.offsetWidth) : headPos.x)
						+ "px";
				...
				function add(left, top, right, height) {
					html += '<div class="CodeMirror-selected" style="position: absolute; left: '
							+ left
							+ 'px; top: '
							+ top
							+ 'px; right: '
				...
				var clientWidth = lineSpace.clientWidth
						|| lineSpace.offsetWidth;
				var clientHeight = lineSpace.clientHeight
						|| lineSpace.offsetHeight;
				if (sel.from.ch && fromPos.y >= 0) {
					var right = sameLine ? clientWidth - toPos.x : 0;
					add(fromPos.x, fromPos.y, right, th);
				}
				var middleStart = Math.max(0, fromPos.y
						+ (sel.from.ch ? th : 0));
				var middleHeight = Math.min(toPos.y, clientHeight)
						- middleStart;
				if (middleHeight > 0.2 * th)
					add(0, middleStart, 0, middleHeight);
				if ((!sameLine || !sel.from.ch)
						&& toPos.y < clientHeight - .5 * th)
		...
		function setSelection(from, to, oldFrom, oldTo) {
			...
			if (from.line != oldFrom) {
				...
				// If there is no non-hidden line left, force visibility on
				// current line
				if (!from1)
		...
		function moveV(dir, unit) {
			var dist = 0, pos = localCoords(sel.inverted ? sel.from : sel.to,
					true);
			if (goalColumn != null)
				pos.x = goalColumn;
			if (unit == "page")
				dist = Math.min(scroller.clientHeight, window.innerHeight
		...
		function indentLine(n, how) {
			...
			var line = getLine(n), curSpace = line.indentation(options.tabSize), curSpaceString = line.text
			...
			if (how == "prev") {
				...
				indentation = mode.indent(state, line.text
						.slice(curSpaceString.length), line.text);
			else if (how == "add")
			...
			return changeLine(handle, function(line, no) {
				if (line.hidden != hidden) {
					...
					if (hidden && (fline == no || tline == no)) {
						...
						// Can't hide the last visible line, we'd have no place
						// to put the cursor
						if (!to)
							return;
						setSelection(from, to);
		...
		function charFromX(line, x) {
			...
			function getX(len) {
				measure.innerHTML = "<pre><span>"
						+ lineObj.getHTML(makeTab, len) + "</span></pre>";
		...
		function endOperation() {
			...
			var tc = textChanged, cbs = callbacks; // these can be reset by
													// callbacks
	...
	CodeMirror.getMode = function(options, spec) {
		...
		if (!mfactory) {
			if (window.console)
				console.warn("No mode " + spec.name
	...
	var commands = CodeMirror.commands = {
		...
		killLine : function(cm) {
			var from = cm.getCursor(true), to = cm.getCursor(false), sel = !posEq(
					from, to);
		...
		goLineStartSmart : function(cm) {
			...
			var text = cm.getLine(cur.line), firstNonWS = Math.max(0, text
					.search(/\S/));
			cm.setCursor(cur.line, cur.ch <= firstNonWS && cur.ch ? 0
					: firstNonWS, true);
		},
	...
	function isModifierKey(event) {
		...
		return name == "Ctrl" || name == "Alt" || name == "Shift"
				|| name == "Mod";
	}
		...
		getHTML : function(makeTab, endAt) {
				...
				if (text.indexOf("\t") == -1) {
					...
					for (var pos = 0;;) {
						...
						if (idx == -1) {
							...
							col += idx - pos;
							var tab = makeTab(col);
							escaped += htmlEscape(text.slice(pos, idx))
									+ tab.html;
							col += tab.width;
							pos = idx + 1;
						}
			...
			if (!allText && endAt == null)
				...
				function advanceMarks() {
					...
					nextChange = markpos < marked.length ? marked[markpos].from
							: Infinity;
					...
					while (true) {
						if (text) {
							...
							for (var j = 0; j < marks.length; ++j)
								appliedStyle = (appliedStyle ? appliedStyle
										+ " " : "")
										+ marks[j].style;
	...
	History.prototype = {
		addChange : function(start, added, old) {
			...
			var time = +new Date, cur = this.done[this.done.length - 1], last = cur
					&& cur[cur.length - 1];
			...
			} else if (last.start > start + old.length
	...
	function selectInput(node) {
		if (ios) { // Mobile Safari apparently has a bug where select() is
					// broken.
